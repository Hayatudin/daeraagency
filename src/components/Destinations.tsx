...
interface Country {
  name: string;
  count: string;
  yOffset: string; // control vertical alignment
  svgFile: string; // filename of the SVG map inside public/countries/
}
...
export default function Destinations() {
  ...
  return (
    <section id="destinations" className="py-24 bg-white border-y border-brand-100/50 relative overflow-hidden">
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 md:px-8">
        ...
        <div className="relative">
            ...
            {row1.map((country) => (
              ...
              >
                ...
                <div className="h-24 flex items-center justify-center mb-4">
                  <div
                    className="w-20 h-20 bg-navy-300 group-hover:bg-brand-500 group-hover:scale-110 transition-all duration-500 drop-shadow-sm filter"
                    style={{
                ...
                <div className="relative w-4.5 h-4.5 rounded-full bg-white border-[4px] border-brand-500 shadow-md group-hover:scale-125 transition-transform duration-300 z-10 my-4">
                  <span className="absolute inset-0.5 rounded-full bg-brand-500 opacity-0 group-hover:opacity-100 transition-opacity"></span>
                </div>

                {/* Text details */}
                <div className="mt-4">
                  <h3 className="font-extrabold text-sm text-navy-950 group-hover:text-brand-600 transition-colors">
                    {country.name}
                  </h3>
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-8 lg:hidden mt-8">
            {[...row1, ...row2].map((country) => (
              <div
                key={country.name}
                className="bg-navy-50/50 border border-brand-100 p-6 rounded-2xl flex flex-col items-center text-center group hover:bg-white hover:border-brand-300 hover:shadow-xl hover:-translate-y-1 transition-all duration-300"
              >
                <div className="w-16 h-16 flex items-center justify-center mb-4 bg-white rounded-xl shadow-sm border border-brand-50">
                  <div
                    className="w-12 h-12 bg-navy-300 group-hover:bg-brand-500 transition-colors duration-500"
                    style={{
                      WebkitMaskImage: `url(/countries/${country.svgFile})`,
                      maskImage: `url(/countries/${country.svgFile})`,
                      WebkitMaskRepeat: "no-repeat",
                ...
                <h3 className="font-extrabold text-sm text-navy-900 group-hover:text-brand-600 transition-colors">
                  {country.name}
                </h3>
        ...
        <div className="mt-16 text-center">
          <a
            href="#contact"
            className="inline-flex items-center gap-2 text-sm font-bold text-brand-600 hover:text-brand-800 transition-colors group/explore"
          >
